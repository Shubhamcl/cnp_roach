...
class CoILICRA_CNP(nn.Module):
    ...
    def __init__(self, im_shape, input_states, acc_as_action,
        ...
        self.context_perception = resnet.get_model(perception_net, im_shape,
                                           num_classes=perception_output_neurons, pretrained=resnet_pretrain)
        # Context measurement block ( 4 extra neurons in layer 1 for: cmd, speed, action)
        self.context_measurements = FC(params={'neurons': [input_states_len+4] + measurements_neurons,
                                       'dropouts': measurements_dropouts,
                                       'end_layer': nn.ReLU})
        
        # Context concat/join block
        self.context_join = Join(params={'after_process':
                                 FC(params={'neurons':
                                            [measurements_neurons[-1] + perception_output_neurons] + join_neurons,
                                            'dropouts': join_dropouts,
                                            'end_layer': nn.ReLU}),
    ...
    def forward(self, im, state, c_im, c_state):
        ...
        # Stack, state both go into perception
        # Context stack, state and FINAL output need to be fed
        # Context measurements should be concated with final (expected) output!
